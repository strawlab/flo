...
fn main() -> Result<()> {
    ...
    flo::run_with_args_and_config(app_options(config), args, flo_config)
}

fn app_options(config: CameraHostConfig) -> flo::AppOptions {
    flo::AppOptions {
        camera_host: Some(Box::new(StrandCamHost {
            config,
...
mod tests {
    ...
    fn integrated_app_disables_legacy_udp_centroid_listener() {
        ...
        assert!(!app_options(config).enable_udp_listener);
    }
